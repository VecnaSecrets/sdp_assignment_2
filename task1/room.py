...
class Room:
    ...
    def check_time(self, start, end):
        ...
        if start > end:
            ...
            return False
        if not is_time_between(time(8, 0), time(21, 0), start.time()) and not is_time_between(time(8, 0), time(21, 0), end.time()):
            return False
        for act in self.activities:
            print(f"fitting {start} - {end} to {act.start} - {act.end}")
